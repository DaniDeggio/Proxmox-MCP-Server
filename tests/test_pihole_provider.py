...
class TestPiHoleIdempotency:
    ...
    async def test_add_new_record(self, pihole: MockPiHoleProvider) -> None:
        result = await pihole.add_dns_record("app.homelab.local", "192.168.1.200")
        assert result["action"] == "created"

    @pytest.mark.asyncio
    async def test_add_duplicate_is_idempotent(self, pihole: MockPiHoleProvider) -> None:
        await pihole.add_dns_record("app.homelab.local", "192.168.1.200")
        result = await pihole.add_dns_record("app.homelab.local", "192.168.1.200")
        assert result["action"] == "already_exists"

    @pytest.mark.asyncio
    async def test_record_exists_check(self, pihole: MockPiHoleProvider) -> None:
        assert await pihole.record_exists("app.homelab.local", "192.168.1.200") is False
        await pihole.add_dns_record("app.homelab.local", "192.168.1.200")
        assert await pihole.record_exists("app.homelab.local", "192.168.1.200") is True

    @pytest.mark.asyncio
    async def test_delete_record(self, pihole: MockPiHoleProvider) -> None:
        await pihole.add_dns_record("temp.homelab.local", "192.168.1.201")
        result = await pihole.delete_dns_record("temp.homelab.local", "192.168.1.201")
        assert result["action"] == "deleted"
        assert await pihole.record_exists("temp.homelab.local", "192.168.1.201") is False
    ...
    async def test_different_ip_same_domain(self, pihole: MockPiHoleProvider) -> None:
        ...
        await pihole.add_dns_record("app.homelab.local", "192.168.1.200")
        result = await pihole.add_dns_record("app.homelab.local", "192.168.1.201")
        assert result["action"] == "created"
        records = await pihole.get_dns_records()
        assert len(records) == 2
